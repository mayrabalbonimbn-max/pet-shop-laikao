...
export function ProductCard({ product }: { product: ProductPreview }) {
  return (
    <article className="surface-default group flex h-full flex-col overflow-hidden border border-brand-100/70 bg-white shadow-[var(--shadow-soft)] transition-all hover:-translate-y-1 hover:border-brand-300 hover:shadow-[var(--shadow-elevated)]">
      <div className="relative flex h-48 items-center justify-center bg-[linear-gradient(135deg,#fffaff_0%,#f2e7ff_58%,#fff0f8_100%)] p-5 text-center">
        <div className="absolute left-4 top-4 flex gap-2">
          {product.featured ? (
            <span className="inline-flex rounded-full bg-[var(--magenta-100)] px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--magenta-600)]">
              Mais vendido
            </span>
          ) : null}
          {product.stockStatus === "low_stock" ? (
            <span className="inline-flex rounded-full bg-[var(--sun-100)] px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-ink-900">
              Últimas unidades
            </span>
          ) : null}
        </div>
      ...
      <div className="flex flex-1 flex-col gap-4 p-5">
        <div className="space-y-2">
          ...
          <p className="text-sm leading-6 text-stone-500">Produto com cara de vitrine forte, pronto para compra rápida, retirada ou entrega.</p>
          <div className="flex items-end justify-between gap-3">
            <p className="font-heading text-2xl font-semibold text-brand-700">{product.priceLabel}</p>
            <span className="rounded-full bg-[var(--sun-100)] px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-ink-900">
              Oferta
            </span>
          </div>
        </div>
        <div className="mt-auto flex items-center justify-between gap-3 border-t border-brand-100/70 pt-4">
